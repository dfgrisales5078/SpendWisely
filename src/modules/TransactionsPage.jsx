...
function TransactionsPage() {
  ...
  const fetchTransactions = useCallback(async () => {
    try {
      const response = await fetch(
        `https://54.162.34.12:443/transactions/?userId=${userId}`
      );
      if (!response.ok) {
  ...
  const handleAddTransaction = async (event) => {
    ...
    if (selectedCategory && amount) {
      ...
      try {
        const response = await fetch("https://54.162.34.12:443/transactions/", {
          method: "POST",
          headers: {
  ...
  const handleDeleteTransaction = async (transactionId) => {
    try {
      await fetch(`https://54.162.34.12:443/transactions/${transactionId}`, {
        method: "DELETE",
      });
